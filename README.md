...
2. LED strip을 사용할 수 있도록 개발할 예정이다.
3. 블루투스를 기반하여 라즈베리끼리 데이터를 공유할 수 있도록 개발할 예정이다.
4. 지속적인 테스트를 통해 신호 패턴의 기준 시간 및 변동 시간의 최적화가 필요로 하다.
5. 현재는 argument를 문자열로 받아와 카운트하는 구역을 그리는데 시간이 된다면 이를 메모장 파일 또는 JSON 파일로 받아와 구역을 그리는 방식으로 변경할 계획이다.
